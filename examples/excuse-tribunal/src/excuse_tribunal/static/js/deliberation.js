...
const TICK_MS = 260;
let theatre;
export function beginDeliberation() {
    byId("deliberation").hidden = false;
    byId("verdict").hidden = true;
    byId("collapse").hidden = true;
    const lights = byId("lights");
    lights.innerHTML = "";
    const bulbs = Array.from({ length: CHARGES_HEARD }, () => lights.appendChild(document.createElement("li")));
    let lit = 0;
    let line = 0;
    theatre = setInterval(() => {
        if (lit < bulbs.length) {
...
export function endDeliberation() {
