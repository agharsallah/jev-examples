...
function hover(f, target, cx, cy, lines) {
...
export function reliabilityChart(bins) {
    ...
    for (const b of bins) {
        ...
        const g = s("g", { class: "hit" }, s("circle", { cx, cy, r: Math.max(12, r + 4), fill: "transparent" }), s("circle", { cx, cy, r, fill: BLUE, stroke: "#131823", "stroke-width": 2 }));
...
export function coverageChart(points, marker) {
    ...
    const line = (key, color) => {
        ...
        const d = pts.map((p, i) => `${i ? "L" : "M"}${f.x(p.threshold)},${f.y(p[key])}`).join(" ");
        f.svg.append(s("path", { d, fill: "none", stroke: color, "stroke-width": 2, "stroke-linejoin": "round" }));
        const last = pts[pts.length - 1];
        if (last)
            f.svg.append(text(f.x(last.threshold) + 8, f.y(last[key]) + 4, key === "coverage" ? "covered" : "agreed", { class: "ax-label" }));
    };
    ...
    for (const p of points) {
        ...
        const g = s("g", { class: "hit" }, s("rect", { x: cx - step / 2, y: f.y(1), width: step, height: f.y(0) - f.y(1), fill: "transparent" }), s("line", { x1: cx, x2: cx, y1: f.y(0), y2: f.y(1), class: "crosshair" }), s("circle", { cx, cy: f.y(p.coverage), r: 4, fill: ORANGE, class: "dot" }), p.accuracy !== null ? s("circle", { cx, cy: f.y(p.accuracy), r: 4, fill: BLUE, class: "dot" }) : null);
    ...
    const legend = h("div", { class: "chart-legend" }, h("span", null, h("i", { style: `background:${ORANGE}` }), "covered"), h("span", null, h("i", { style: `background:${BLUE}` }), "agreed, among those covered"));
    f.wrap.append(legend);
    return f.wrap;
}
/** Rows are the label on the issue, columns Jev's pick. One blue ramp, by row share. */
export function confusionTable(labels, cells, names = {}) {
    const shown = labels.slice(0, 12);
    return h("div", { class: "confusion-wrap" }, h("table", { class: "confusion" }, h("thead", null, h("tr", null, h("th", { class: "corner" }, "on it ↓  Jev →"), shown.map((l) => h("th", { title: l }, h("span", null, names[l] ?? l))))), h("tbody", null, shown.map((truth, i) => {
        const row = cells[i] ?? [];
        const total = row.reduce((a, b) => a + b, 0) || 1;
        return h("tr", null, h("th", { title: truth }, names[truth] ?? truth), shown.map((pick, j) => {
            const n = row[j] ?? 0;
            const share = n / total;
