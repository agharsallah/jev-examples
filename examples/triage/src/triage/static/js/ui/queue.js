...
export function queueBoard(scan, rows, lanes, onOpen) {
    ...
    return h("div", { class: "queue" }, h("div", { class: "queue-stats" }, stat(String(rows.length), "open issues read"), stat(String(t.questions.toLocaleString()), "questions asked"), stat(dollars(t.cost_usd), `${t.input_tokens.toLocaleString()} tokens`), stat(`${t.mean_latency_s.toFixed(2)}s`, "per issue"), stat(String(drift), "with label drift"), stat(String(dups), "likely duplicates")), t.skipped.length
        ? h("p", { class: "faint small" }, `${t.skipped.length} refused by the API's firewall: `, t.skipped.map((s) => `#${s.number}`).join(", "))
        : null, h("div", { class: "lanes-board" }, LANE_ORDER.filter((lane) => byLane.has(lane)).map((lane) => {
        const items = (byLane.get(lane) ?? []).sort((a, b) => b.assessment.priority - a.assessment.priority);
        return h("section", { class: `lane-col lane-col-${lane}` }, h("header", null, h("span", { class: `lane lane-${lane}` }, lanes[lane] ?? lane), h("span", { class: "faint" }, String(items.length))), items.map((row) => queueCard(row, onOpen)));
    })));
}
function stat(value, label) {
    return h("div", { class: "stat" }, h("strong", null, value), h("span", null, label));
}
function queueCard(row, onOpen) {
