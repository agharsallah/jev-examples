...
export function controls(settings, defaults, onChange) {
    ...
    const slider = (value, set, label, help) => {
    ...
    return h("section", { class: "card" }, h("header", { class: "card-head" }, h("h3", null, "Policy knobs"), h("span", { class: "card-note" }, "rescored locally · no new request")), ...KNOBS.map(([key, label, help]) => slider(current[key], (v) => (current[key] = v), label, help)), h("div", { class: "eyebrow" }, "priority weights"), ...WEIGHTS.map(([key, label]) => slider(current.weights[key] ?? 0, (v) => (current.weights[key] = v), label, `weight on the ${label} score`)), h("button", {
        class: "ghost small",
        type: "button",
