...
function unitAnswer(m, unit, lens) {
    if (lens === "off")
        return null;
...
export function drawSheet(host, m, a, lens, inspector, hooks) {
    const body = m.body;
    const parts = [];
    let cursor = 0;
...
function inspect(panel, m, unit, hooks, sheet, el) {
    ...
    fill(panel, h("div", { class: "inspector-head" }, h("span", { class: "eyebrow" }, `unit ${unit.index} · ${unit.kind}`), h("button", { class: "ghost small", type: "button", onclick: () => { panel.hidden = true; el.classList.remove("focused"); } }, "close")), h("blockquote", null, unit.text.length > 280 ? `${unit.text.slice(0, 280)}…` : unit.text), kind ? h("div", null, h("h4", null, "what kind of report this part signals"), distribution(kind.probabilities, kind.choice, { limit: 4 })) : null, comp ? h("div", null, h("h4", null, "which part of the project it points to"), distribution(comp.probabilities, comp.choice, { limit: 4 })) : null, h("p", { class: "faint small" }, "Each unit is asked on its own, with the whole issue still in the state. ", "Removing it re-asks the decisions without it — a measured answer to ‘did this matter?’."), h("button", { class: "primary small", type: "button", onclick: () => hooks.onRemove([unit.index]) }, "Remove this and re-ask"));
    panel.hidden = false;
}
