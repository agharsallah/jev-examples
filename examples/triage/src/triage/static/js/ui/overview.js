...
export function flags(row, k, dupes) {
    const out = new Set();
    const decided = row.needs_decision < 0.5;
    if (row.readiness >= k.readyAt && decided && !BLOCKED.has(row.lane))
        out.add("ready");
    if (row.beginner >= k.beginnerAt && decided && row.security < 0.5 && !["private", "close", "duplicate"].includes(row.lane))
        out.add("beginner");
    if (row.needs_decision >= 0.6)
        out.add("decision");
    if (row.lanes.includes("needs_info"))
        out.add("ask");
    if (dupes.has(row.number))
        out.add("duplicate");
    if (row.lanes.includes("private"))
        out.add("security");
    if (row.lanes.includes("close"))
        out.add("close");
    if (row.drift.length)
        out.add("drift");
    return out;
}
export function quadrant(row, k) {
...
export function clusters(data, dupAt) {
    ...
    for (const e of strong) {
        const root = find(e.a);
        const c = groups.get(root) ?? { members: [], edges: [], titles: new Map() };
        c.edges.push(e);
    ...
    for (const c of groups.values()) {
        ...
        for (const e of c.edges) {
            ...
            if (!best.has(key) || best.get(key).same < e.same)
                best.set(key, e);
        }
...
// -- the page -------------------------------------------------------------
export function overviewView(data, k, f, sort, hooks) {
    ...
    const visible = data.rows.filter((r) => (!f.flag || flagged.get(r.number).has(f.flag)) &&
    ...
    return h("div", { class: "overview" }, tiles(data, flagged, groups, f, hooks), filterBar(f, visible.length, data.rows.length, hooks), h("div", { class: "ov-grid" }, h("section", { class: "card ov-map" }, head("The map", "impact against readiness · one dot per issue · click one to open it"), mapChart(data.rows, shown, k, f, hooks), kindLegend("click a quadrant to filter")), h("section", { class: "card" }, head("Where the work is", "Jev's component pick × kind · click a bar to filter"), breakdownChart(visible, f, hooks), kindLegend("a segment filters to that component and kind"))), shortlists(visible, flagged, hooks), duplicatesPanel(groups, shown, f, hooks), explorer(visible, flagged, sort, hooks), knobsCard(data, k, hooks));
}
function head(title, note) {
    return h("header", { class: "card-head" }, h("h3", null, title), h("span", { class: "card-note" }, note));
}
// -- tiles ----------------------------------------------------------------
function tiles(data, flagged, groups, f, hooks) {
    const count = (flag) => [...flagged.values()].filter((s) => s.has(flag)).length;
    const t = data.totals;
    const tile = (flag, value, extra) => h("button", {
    ...
    return h("div", { class: "tiles" }, h("div", { class: "tile static" }, h("strong", null, String(data.repo.read)), h("span", null, "open issues read"), h("small", null, `${t.questions.toLocaleString()} questions · ${dollars(t.cost_usd)}${data.repo.read < data.repo.open_issues ? ` · newest ${data.repo.read} of ${data.repo.open_issues}` : ""}`)), tile("ready", String(count("ready"))), tile("beginner", String(count("beginner"))), tile("decision", String(count("decision"))), tile("ask", String(count("ask"))), tile("duplicate", String(count("duplicate")), `${groups.length} cluster${groups.length === 1 ? "" : "s"}`), tile("security", String(count("security"))), tile("close", String(count("close"))), tile("drift", String(count("drift"))));
}
function filterBar(f, shown, total, hooks) {
    const chips = [];
    const chip = (text, clear) => chips.push(h("button", { class: "chip filter", type: "button", onclick: () => hooks.filter(clear) }, text, h("i", null, "×")));
    if (f.flag)
        chip(FLAG_WORDS[f.flag][0], { flag: null });
    if (f.quadrant)
        chip(QUADRANT_WORDS[f.quadrant][0], { quadrant: null });
    if (f.kind)
        chip(`kind: ${f.kind}`, { kind: null });
    if (f.component)
        chip(`component: ${f.component}`, { component: null });
    ...
    return h("div", { class: "filter-bar" }, search, h("div", { class: "chips" }, chips), h("span", { class: "faint small" }, shown === total ? `all ${total} issues` : `${shown} of ${total} issues`), chips.length ? h("button", { class: "ghost small", type: "button", onclick: () => hooks.filter({ flag: null, kind: null, component: null, quadrant: null, q: "" }) }, "clear filters") : null);
}
// -- the map --------------------------------------------------------------
...
function mapChart(rows, shown, k, f, hooks) {
    ...
    for (const r of ordered) {
        ...
        const g = s("g", { class: `pt${on ? "" : " dim"}`, tabindex: on ? 0 : -1 }, s("circle", { cx, cy, r: 10, fill: "transparent" }), s("circle", { cx, cy, r: 4.5, fill: KIND_COLOR[r.kind] ?? KIND_COLOR.other, stroke: "#131823", "stroke-width": 1.5 }));
        const show = () => {
            tip.replaceChildren(h("div", null, h("strong", null, `#${r.number}`), " ", h("span", null, r.title.length > 70 ? `${r.title.slice(0, 70)}…` : r.title)), h("div", null, h("strong", null, two(r.readiness)), h("span", null, " readiness · "), h("strong", null, two(r.impact)), h("span", null, ` impact · ${r.kind}`)));
        ...
        g.addEventListener("keydown", (e) => { if (e.key === "Enter")
            hooks.open(r.number); });
        svg.append(g);
    }
    for (const text of quadLabels)
        svg.append(text);
    const wrap = h("div", { class: "chart-wrap map-wrap" });
    wrap.append(svg, tip);
    return wrap;
}
function kindLegend(note) {
    return h("div", { class: "chart-legend" }, KINDS.map((kind) => h("span", null, h("b", { class: "dot-key", style: `background:${KIND_COLOR[kind]}` }), kind)), h("span", { class: "faint" }, `· ${note}`));
}
// -- breakdown: component × kind stacked bars --------------------------------
function breakdownChart(rows, f, hooks) {
    const by = new Map();
    for (const r of rows) {
    ...
    if (!bars.length)
        return h("p", { class: "faint" }, "Nothing matches the filters.");
    return h("div", { class: "bars" }, bars.map((b) => h("div", { class: `bar-row${f.component === b.comp ? " active" : ""}` }, h("button", { class: "bar-name link", type: "button", title: b.comp, onclick: () => hooks.filter({ component: f.component === b.comp ? null : b.comp }) }, b.comp), h("div", { class: "bar-track" }, h("div", { class: "bar-stack", style: `width:${(b.total / most) * 100}%` }, KINDS.filter((kind) => b.kinds.get(kind)).map((kind) => h("button", {
        class: "seg",
        type: "button",
...
// -- shortlists -------------------------------------------------------------
function shortlists(rows, flagged, hooks) {
...
// -- duplicates -------------------------------------------------------------
function duplicatesPanel(groups, shown, f, hooks) {
    const visible = f.flag || f.kind || f.component || f.quadrant || f.q
        ? groups.filter((c) => c.members.some((n) => shown.has(n)))
        : groups;
    return h("section", { class: "card" }, head("Duplicate clusters", "issues Jev reads as the same underlying problem · linked by ‘same problem’ probability"), visible.length
...
function clusterCard(c, hooks) {
    ...
    return h("div", { class: "cluster" }, clusterGraph(c), closed.length
...
function clusterGraph(c) {
    ...
    const pos = new Map(c.members.map((m, i) => {
        ...
        return [m, [cx + (n === 2 ? 55 : R) * Math.cos(angle), cy + (n === 2 ? 0 : R) * Math.sin(angle)]];
    }));
...
function explorer(rows, flagged, sort, hooks) {
    const sorted = [...rows].sort((a, b) => sort === "number" ? b.number - a.number
...
function knobsCard(data, k, hooks) {
    ...
    const weights = (w) => Object.entries(w).map(([name, v]) => `${pct(v)} ${name}`).join(" + ");
    return h("section", { class: "card" }, head("How these are computed", "plain code over Jev's answers · the knobs recompute in the browser"), slider("readyAt", "ready to pick up at readiness ≥"), slider("beginnerAt", "good first issue at beginner ≥"), slider("dupAt", "duplicates when ‘same problem’ ≥"), h("dl", { class: "defs" }, h("dt", null, "readiness"), h("dd", null, weights(data.weights.readiness), ". ‘decided’ is 1 − needs a decision; ‘reproducible’ counts only for bugs."), h("dt", null, "beginner"), h("dd", null, weights(data.weights.beginner), ". ‘small’ is 1 − scope."), h("dt", null, "impact"), h("dd", null, "Jev's impact Score, 0 (cosmetic) to 1 (data loss or unusable)."), h("dt", null, "ready to pick up"), h("dd", null, "readiness over the bar, no open decision, and not in a private, human, ask-the-reporter, duplicate or close lane.")));
}
