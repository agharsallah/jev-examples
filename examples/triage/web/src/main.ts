...
import { type Filters, type Knobs, type SortKey, overviewView } from "./ui/overview.js";
import { queueBoard } from "./ui/queue.js";
import { type Lens, drawSheet } from "./ui/sheet.js";
import { taxonomyPanel } from "./ui/taxonomy.js";

const el = {
  form: byId<HTMLFormElement>("go"),
  ref: byId<HTMLInputElement>("ref"),
  fresh: byId<HTMLInputElement>("fresh"),
  examples: byId("examples"),
  status: byId("status"),
  repoBar: byId("repoBar"),
  repoName: byId("repoName"),
  repoMeta: byId("repoMeta"),
  labelsToggle: byId<HTMLButtonElement>("labelsToggle"),
  ...
  queueLimit: byId<HTMLInputElement>("queueLimit"),
  queueRun: byId<HTMLButtonElement>("queueRun"),
  queueBoard: byId("queueBoard"),
  queueKnobs: byId("queueKnobs"),
  evalView: byId("evalView"),
  evalLimit: byId<HTMLInputElement>("evalLimit"),
  evalRun: byId<HTMLButtonElement>("evalRun"),
  evalBody: byId("evalBody"),
  overviewView: byId("overviewView"),
  overviewRunRow: byId("overviewRunRow"),
  overviewIntro: byId("overviewIntro"),
  overviewLimit: byId<HTMLInputElement>("overviewLimit"),
  overviewRun: byId<HTMLButtonElement>("overviewRun"),
  overviewProgress: byId("overviewProgress"),
  overviewSnapshot: byId("overviewSnapshot"),
  overviewAge: byId("overviewAge"),
  overviewRefresh: byId<HTMLButtonElement>("overviewRefresh"),
  overviewBody: byId("overviewBody"),
};

type View = "overview" | "issue" | "queue" | "eval";
...
function parseRef(raw: string): { repo: string; number: number | null; view: View | null } | null {
  const match = raw
    .trim()
...
function show(view: View): void {
  state.view = view;
  el.tabs.querySelectorAll<HTMLButtonElement>("button").forEach((b) => b.classList.toggle("active", b.dataset.view === view));
...
function setHash(repo: string, tail: string | number | null): void {
  history.replaceState(null, "", `#${repo}${tail !== null ? `/${tail}` : ""}`);
}

function refFromHash(): string {
  return decodeURIComponent(location.hash.replace(/^#/, ""));
}

// -- drawing -------------------------------------------------------------

function drawRepo(): void {
  const r = state.repo;
  if (!r) return;
...
function drawIssue(): void {
  ...
  fill(
    el.cards,
    verdictCard(m, a, meta),
    whyCard(m, a, state.counterfactuals, state.asking, () => runCounterfactual(null)),
    readingCard(m, meta, state.settings),
    checklistCard(m, a, state.settings),
    duplicatesCard(a, state.settings),
  );
...
async function rescore(settings: Settings, reset = false): Promise<void> {
...
async function runCounterfactual(removals: number[][] | null): Promise<void> {
  if (!state.issue || !state.assessment || state.asking) return;
...
async function open(raw: string, fresh = false): Promise<void> {
  ...
  try {
    ...
    if (!parsed.number) {
      ...
      if (!parsed.view) setHash(parsed.repo, "overview");
      primeOverview();
...
async function loadSavedOverview(): Promise<void> {
  if (!state.repo || state.overview) return;
  const slug = state.repo.repo.slug;
  const found = await overviewApi.saved(slug).catch(() => null);
  if (!found || state.repo?.repo.slug !== slug) return;
...
function drawOverview(): void {
  ...
  el.overviewBody.replaceChildren(
    overviewView(state.overview, state.knobs, state.filters, state.sort, {
      open: openIssue,
      filter: (patch) => {
        ...
        drawOverview();
        if (typing) {
          const search = el.overviewBody.querySelector<HTMLInputElement>('input[type="search"]');
          search?.focus();
...
async function runOverview(refresh = false): Promise<void> {
  ...
  const fill = el.overviewProgress.querySelector<HTMLElement>(".progress-fill")!;
  const text = el.overviewProgress.querySelector<HTMLElement>("span")!;
  ...
  try {
    ...
    for (;;) {
      ...
      const status = await overviewApi.poll<Overview>(job);
      if (status.total) {
        fill.style.width = `${(status.done / status.total) * 100}%`;
        text.textContent = `read ${status.done} of ${status.total} issues`;
      }
      if (status.status === "failed") throw new Error(status.error ?? "The overview failed.");
...
async function runQueue(): Promise<void> {
  ...
  try {
    ...
    drawQueue();
    el.queueKnobs.replaceChildren(
      controls(state.settings, state.meta!.defaults, (settings, reset) => void rescoreQueue(settings, reset)),
    );
  } catch (error) {
    failed(error);
  } finally {
...
async function rescoreQueue(settings: Settings, reset = false): Promise<void> {
  ...
  try {
    ...
    if (reset) {
      el.queueKnobs.replaceChildren(
        controls(state.settings, state.meta!.defaults, (s, r) => void rescoreQueue(s, r)),
      );
    }
  } catch (error) {
    failed(error);
...
async function runEval(): Promise<void> {
  ...
  try {
    ...
    idle();
    const applyAt = (state.settings?.choice_apply as number | undefined) ?? 0.7;
...
async function start(): Promise<void> {
  ...
  fill(
    el.examples,
    "try ",
    state.meta.examples.flatMap((ref, i) => [
      i ? " · " : "",
