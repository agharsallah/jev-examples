...
export function markDraft(draft: string, sentences: MarkedSentence[]): Markup {
  let html = "";
  let cursor = 0;
...
function legendEntry(probe: Probe): HTMLElement {
  const entry = document.createElement("span");
  entry.innerHTML = `<i class="m-${probe}"></i> ${escapeHtml(probeLabel(probe))}`;
  return entry;
}

export function drawMarkup(draft: string, sentences: MarkedSentence[]): void {
