...
import { askMove, type Plan } from "../api.js";
import { currentLevel } from "../game/speed.js";
import { turns } from "../game/pieces.js";
import { W, asRows, width, type Block, type Shape } from "../game/rules.js";
import type { Supply } from "../game/supply.js";
import { setTag } from "../ui/dom.js";
import { drawMini, type Screen } from "../ui/screen.js";
import type { ThinkingCard } from "../ui/thinking.js";
import { Player, type Hooks, type WellEls } from "./player.js";

type State = "idle" | "thinking" | "lining up" | "dropping" | "clearing" | "error";

export class Bot extends Player {
  private state: State = "idle";
  private plan: Plan | null = null;
  private shape: Shape | null = null;
  private target = { rot: 0, x: 0, y: 0 };
  private rot = 0;
  private x = 0;
  private y = 0;
  private keying = 0;
  private run = 0; // a restart makes any answer still in flight stale
  ...
  private async think(): Promise<void> {
    ...
    try {
      ...
      if ("error" in answer) return this.offline(answer.error);
      if ("game_over" in answer) return this.topOut();
      this.card.show(answer, Math.round(performance.now() - started), this.name);
      this.take(answer);
    } catch (error) {
      if (run === this.run) this.offline(`Could not reach the server: ${(error as Error).message}`);
    }
  }

  /** Put the piece at the top the way you get it, and remember where it has to end up. */
  private take(plan: Plan): void {
    this.plan = plan;
    this.rot = 0;
  ...
  private offline(message: string): void {
  ...
  private cells(): Block[] {
    ...
    return this.shape.map(([cx, cy]): Block => [cx + this.x, cy + this.y, piece]);
  }

  private lock(): void {
    // The plan is what actually gets written in: the walk across the top is theatre.
    const plan = this.plan!;
    this.settle(plan.cells.map(([x, y]): Block => [x, y, plan.piece]));
    this.plan = null;
    this.shape = null;
  ...
  paint(): void {
    if (this.clearing) this.screen.paint(this.grid, [], [], this.clearing);
      ...
      const piece = this.plan.piece;
      const ghost = this.plan.cells.map(([x, y]): Block => [x, y, piece]);
