...
import { H, W, type Block, type Grid, type PieceKey } from "../game/rules.js";

export class Screen {
  readonly el: HTMLElement;
  private readonly cells: HTMLElement[] = [];
  private readonly last: string[] = Array<string>(W * H).fill("");
  ...
  paint(grid: Grid, active: Block[] = [], ghost: Block[] = [], flash: number[] = []): void {
    const klass = Array<string>(W * H).fill("");
    const colour = Array<string>(W * H).fill("");
    grid.forEach((row, y) =>
      row.forEach((piece, x) => {
        if (piece) {
          klass[y * W + x] = "cell on";
    ...
    for (const y of flash) {
      for (let x = 0; x < W; x++) klass[y * W + x] = "cell flash";
    }
    klass.forEach((name, i) => {
      const want = (name || "cell") + colour[i];
      if (this.last[i] === want) return;
...
export function drawMini(el: HTMLElement, piece: PieceKey | null): void {
  el.innerHTML = "";
  if (!piece) return;
