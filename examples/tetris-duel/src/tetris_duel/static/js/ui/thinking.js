...
export class ThinkingCard {
    ...
    static copy() {
        ...
        if (!root) {
            ...
            root = original.cloneNode(true);
            root.id = COPY_ID;
            root.querySelectorAll("[id]").forEach((el) => (el.id = `${COPY_PREFIX}${el.id}`));
            original.after(root);
        }
    ...
    reset(name, side) {
        ...
        for (const name of ["danger", "clear", "slot", "conf"])
            this.meter(name, 0, "—");
    ...
    show(plan, ms, who) {
        ...
        if (plan.overruled)
            overrule.textContent = plan.note;
        const bars = this.q("bars");
        bars.innerHTML = "";
        for (const option of plan.options)
            bars.appendChild(bar(option));
...
function pickLine(plan, who) {
    return (`<b>${plan.piece}</b> → ${plan.where} · chosen from <b>${plan.menu_size}</b> legal landings` +
